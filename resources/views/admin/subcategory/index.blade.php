<x-LayoutAdmin>
    <div class="bg-[#0f172a]">
        <div class="w-full mx-auto">
            <div class="flex justify-between items-center mb-6">
                <p class="text-2xl font-bold text-white">Sub Categories Management</p>
            ...
            <div class="bg-[#1e293b] rounded-xl mb-4">
                <div class="p-6">
                    <form method="GET" action="{{ route('admin.subcategories.index') }}" class="space-y-4"
                        id="searchForm">
                            ...
                            @forelse($subCategories as $subCategory)
                                <tr class="hover:bg-[#334155] duration-200">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-white">{{ $subCategory->title }}</div>
                                    </td>

                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-white">{{ $subCategory->category->title }}</div>
                                    </td>

                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $subCategory->status === 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-left text-sm font-medium">
                                        <div class="flex items-center justify-start space-x-2">
                                            <!-- Edit Sub Category -->
                                            <button
                                                onclick="openEditModal('{{ $subCategory->id }}', '{{ $subCategory->title }}', '{{ $subCategory->category_id }}', '{{ $subCategory->status }}')"
                                            ...
                                            <!-- Delete Sub Category -->
                                            <button
                                                onclick="openDeleteModal('{{ route('admin.subcategories.destroy', $subCategory->id) }}')"
                                ...
                                <tr>
                                    <td colspan="4" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <h3 class="text-lg font-medium text-white mb-2">No sub categories found</h3>
                                            <p class="text-gray-400 mb-4">Try adding a new sub category or adjust your
                                                search.</p>
                                        </div>
                                    </td>
                                </tr>
    ...
    <div id="createModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden items-center justify-center z-50">
        <div class="bg-[#1e293b] rounded-lg p-6 max-w-sm mx-4 w-full">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-medium text-white">Add New Sub Category</h3>
            ...
            <form action="{{ route('admin.subcategories.store') }}" method="POST">
                ...
                <div class="space-y-4">
                    <div>
                        <label for="title" class="block text-sm font-medium text-white mb-2">Sub Category
                            Title</label>
                    ...
                    <div>
                        <label for="category_id" class="block text-sm font-medium text-white mb-2">Parent
                            Category</label>
                        <select name="category_id" id="category_id"
                            class="block w-full px-3 py-2.5 bg-[#0f172a] border border-gray-600 rounded-lg text-white
                            focus:outline-none focus:ring-2 focus:ring-gray-500 focus:border-gray-500
                            duration-200 @error('category_id') border-red-500 @enderror">
    ...
    <div id="editModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden items-center justify-center z-50">
        <div class="bg-[#1e293b] rounded-lg p-6 max-w-sm mx-4 w-full">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-medium text-white">Edit Sub Category</h3>
            ...
            <form id="editForm" method="POST">
                ...
                <div class="space-y-4">
                    <div>
                        <label for="edit_title" class="block text-sm font-medium text-white mb-2">Sub Category
                            Title</label>
                    ...
                    <div>
                        <label for="edit_category_id" class="block text-sm font-medium text-white mb-2">Parent
                            Category</label>
    ...
    <div id="deleteModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden items-center justify-center z-50">
        <div class="bg-[#1e293b] rounded-lg p-6 max-w-sm mx-4">
            <div class="flex items-center mb-4">
                ...
                <div class="ml-3">
                    <h3 class="text-lg font-medium text-white">Delete Sub Category</h3>
                </div>
            </div>
            <p class="text-sm text-gray-400 mb-4">Are you sure you want to delete this sub category? This action cannot
                be
                undone.</p>
            <div class="flex justify-end space-x-3">
    ...
    <script>
        ...
        function openEditModal(id, title, categoryId, status) {
            ...
            document.getElementById('editForm').action = `/admin/subcategories/${id}`;
